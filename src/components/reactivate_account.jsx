...
export default function Reactivateaccount() {
    ...
    const delete_account=async(e)=>{
        ...
        setLoading(true)
        setTimeout(async() => {
            await axios.post('https://imitre-backend-cn8ls.ondigitalocean.app/reactivateaccount/',{
...
className=' outline-none w-full'
 ...
 />
                ...
                {loading?
                    <PulseLoader color='#009379' size={32} className="xs:mt-[20px] lg:mt-[0px] lg:w-[320px] md:w-[160px] xs:w-[160px] md:mb-[40px] translate-x-3"/>
                    :
                    <button className='poppins-bold xs:w-auto xs:h-auto xs:py-[16px] xs:px-[50px] rounded-[20px] bg-learn_more text-white hover:scale-110 transition-all duration-300 ease-in-out md:mb-[40px] xs:mb-10'>
                        Reactivate Account
                    </button>}
